<script>
	...
	let hours = $state(Math.floor(episode.duration / 3600));
	let minutes = $state(Math.floor((episode.duration % 3600) / 60));
	let seconds = $state(episode.duration % 60);

    let synopsis = $state(episode.synopsis);

    let fileLink = $state(episode.filelink);

    let streamingLink = $state(episode.streaminglink);
    ...
    let numberOfCurators = $state(episode.curators.length);
    let curators = $state([]);
...
<div id="titleDiv" class="mBottom-xl">
	<a class="iconSize2" href="/studio/programs/{program.id}/{episode.number}">
		<img class="iconSize2" src={undoIcon} alt="Undo Icon" />
	</a>
	<h1 class="tSize3">Editar Episódio <b class="cMain3">({program.title}/{episode.title})</b></h1>
</div>
...
<form method="POST" action="?/editEpisode">
    ...
    <div class="mTop-xl bRadius1">
		...
		<label for="synopsis">
			...
			<textarea id="synopsis" name="synopsis" class="bRadius1" bind:value={synopsis}></textarea>
		</label>
        <label for='fileLink'>
            <h3 class="mTop-xl"><b class="bold cRed">·</b> Link de download (Google Drive)</h3>
            <input type='text' id='fileLink' name='fileLink' class='solidBorder bRadius1' bind:value={fileLink} required />
        </label>
					...
					<select id="curators{index}" bind:value={curators[index]} class="bRadius1" required>
						{#each members as member}
							{#if !curators.slice(0, index).includes(member.account_id)}
								<option 
									selected={episode.curators[index] == member.name ? true : undefined}
